...
// Add to cart counter
let cartCount = 0;
document.querySelectorAll('.add-to-cart-btn, .product-quick-add').forEach(btn => {
  btn.addEventListener('click', () => {
    cartCount++;
    const counter = document.querySelector('.cart-count');
    if (counter) counter.textContent = cartCount;
    btn.textContent = '✓ Added';
    setTimeout(() => btn.textContent = btn.classList.contains('add-to-cart-btn') ? 'Add to Cart' : 'Quick Add', 1500);
  });
});
